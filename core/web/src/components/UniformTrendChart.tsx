...
import { CartesianGrid, LabelList, Line, LineChart, XAxis, YAxis, Tooltip, ReferenceLine } from "recharts"
...
export default function UniformTrendChart({ tipo = 'hashtag1' }: TendenciaUniformeProps) {
  const datosActuales = datosPorTipo[tipo];
  
  const calcularPromedioGeneral = () => {
    const promediosPorPeriodo: { periodo: string; promedio: number }[] = [];
    
    for (let colIndex = 1; colIndex <= 4; colIndex++) {
      const valoresPeriodo = datosActuales.map((fila: (string | number)[]) => Number(fila[colIndex]));
      const suma = valoresPeriodo.reduce((acc: number, curr: number) => acc + curr, 0);
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Análisis de tendencias y métricas</CardDescription>
      </CardHeader>
      <CardContent>
